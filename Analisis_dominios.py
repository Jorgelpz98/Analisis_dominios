...
def main(domain, masscanOutput, urlOutput):
    ...
    if (not masscanOutput and not urlOutput):
        print("[+]: Descargando lista de subdominios desde crt.sh. . .")
    response = collectResponse(domain)
    if (not masscanOutput and not urlOutput):
        print("[+]: Descarga de lista de subdominios completada.")
    domains = collectDomains(response)
    if (not masscanOutput and not urlOutput):
        print("[+]: Subdominios % analizados desde la lista." % len(domains))
    if len(domains) == 0:
        exit(1)
...
def collectResponse(domain):
    ...
    try:
        ...
        print("[!]: Conexión al servidor fallida.")
        exit(1)
    try:
        domains = response.json()
        return domains
    except:
        print("[!]:El servidor no respondió con un json válido.")
        exit(1)
